...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, **kwargs):
        ...
        self.malicious_patterns: List[Pattern] = [
            ...
            re.compile(r'\b(?:select|insert|update|delete|drop|truncate|--|/\*|\*/|@@|@|char\(|or\s+1=1|waitfor\s+delay)\b', 
                     re.IGNORECASE),  # SQL Injection
            re.compile(r'\b(?:document\.cookie|eval\(|alert\(|onload=|onerror=|onclick=)', 
                     re.IGNORECASE),  # XSS
            re.compile(r'\b(?:union\s+select|exec\s*\(|sp_|xp_|;--|/\*!|@@version)\b', 
                     re.IGNORECASE),  # More SQLi
        ]
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if content_length and int(content_length) > self.max_file_size:
            return JSONResponse(
                status_code=413,
                content={"detail": f"File size exceeds maximum allowed size of {self.max_file_size} bytes"}
            )
        ...
        if 'multipart/form-data' in content_type:
            if not await self._validate_file_upload(request):
                return JSONResponse(
                    status_code=400,
                    content={"detail": "Invalid file type or content"}
                )
    ...
    def _check_dict_for_malicious_input(self, data: Any) -> bool:
        ...
        if isinstance(data, dict):
            return any(
                self._is_malicious(str(key)) or self._check_dict_for_malicious_input(value)
