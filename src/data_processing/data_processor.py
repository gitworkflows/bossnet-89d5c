...
This module provides core functionality for processing educational data from various
sources including BANBEIS, Education Board, and other institutional data sources.
"""
...
class DataProcessor:
    ...
    def calculate_performance_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in metrics_df.columns for col in ['days_present', 'total_school_days']):
            metrics_df['attendance_rate'] = (
                metrics_df['days_present'] / metrics_df['total_school_days']
            ).round(3)
